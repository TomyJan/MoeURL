...
if ($databaseQuery['num'] == 0) {
    ...
    if ($stmt->execute()) {
        ...
        exit('{"code":1,"msg":"success","result":"' . get_http_type() . 'zi5.cc/' . $rewriteStr . $shortCode . '"}');
    }
} else {
    //exit('{"code":1,"msg":"success","result":"' . get_http_type() . $_SERVER['HTTP_HOST'] . '/' . $rewriteStr . $databaseQuery['result'][0]['code'] . '"}');
    exit('{"code":1,"msg":"success","result":"' . get_http_type() . 'zi5.cc/' . $rewriteStr . $databaseQuery['result'][0]['code'] . '"}');
}
