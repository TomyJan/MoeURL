...
describe('pages', () => {
  ...
  it('renders own links states and row actions', async () => {
    ...
    expect(screen.getByTestId('console-page-mobile-list')).toBeTruthy()
    ...
    const mobileList = screen.getByTestId('console-page-mobile-list')
    const mobileCards = mobileList.querySelectorAll('.console-page__mobile-card')
    await fireEvent.click(within(mobileCards[0] as HTMLElement).getByRole('button', { name: '禁用' }))
    await fireEvent.click(within(mobileCards[0] as HTMLElement).getByRole('button', { name: '复制' }))
    await fireEvent.click(within(mobileCards[0] as HTMLElement).getByRole('button', { name: '删除' }))
  ...
  it('renders admin links states and row actions', async () => {
    ...
    const mobileList = screen.getByTestId('console-page-mobile-list')
    const mobileCards = mobileList.querySelectorAll('.console-page__mobile-card')
    await fireEvent.click(within(mobileCards[0] as HTMLElement).getByRole('button', { name: '启用' }))
    await fireEvent.click(within(mobileCards[0] as HTMLElement).getByRole('button', { name: '复制' }))
    await fireEvent.click(within(mobileCards[0] as HTMLElement).getByRole('button', { name: '删除' }))
