...
export const componentStubs: Record<string, Component> = {
  ...
  VTextField: {
    props: ['autocomplete', 'disabled', 'errorMessages', 'label', 'modelValue', 'placeholder', 'step', 'type'],
    emits: ['update:modelValue', 'keyup'],
    template: '<label>{{ label }}<input :aria-label="label" :autocomplete="autocomplete" :disabled="disabled" :placeholder="placeholder" :step="step" :type="type || \'text\'" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" @keyup="$emit(\'keyup\', $event)" /><span v-if="errorMessages">{{ errorMessages }}</span></label>',
  },
}
