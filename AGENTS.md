...
当前已完成到 v0.8.0 多域名管理的功能实现与本地可执行门禁。v0.6.0 生产验收、v0.7.0 目标 CI/race/外部 OIDC 互操作，以及 v0.8.0 目标 CI/race/安全扫描/真实 DNS/TLS/隔离恢复和生产发布验收仍待目标环境证据。v0.8.0 工作以 `docs/product/scope-v0.8.0.md`、域名规格、设计、实施计划和验收清单为准；本地全量 Playwright 与 Compose smoke 结果不替代目标环境发布证据。
...
如果任务涉及 review comments、CI 或测试失败、覆盖率门禁、文档与代码一致性、生成文件漂移或迁移回滚，也必须继续阅读该交付规范。
...
- `AGENTS.md` 只沉淀稳定的项目约束和工作入口；详细执行流程写入 `docs/implementation/agent-delivery-guidelines.md`，版本特有行为写入对应 scope、spec 和 implementation 文档。
- 对话中的临时要求只有在可跨任务复用、与现有产品和工程规则一致时才沉淀为全局规范；具体 review 行号、一次性故障和单版本实现细节不得写入全局提示。
...
- 验收状态必须与实际证据一致；本地运行时、CI 目标运行时、提交 SHA、运行链接、命令和环境变量不得相互替代或混写。
- 覆盖率问题必须通过有效测试、必要文档注释或正确实现解决，不得降低阈值、无依据扩大排除范围、排除可测试行为或用无意义用例刷门禁。
- SQLC、权限目录快照等生成文件必须通过仓库约定的生成入口更新，不得手工编辑；生成后必须验证零漂移。
- 测试必须可重复且相互隔离；优先使用确定性同步和清理回调，不依赖固定休眠、跨测试顺序或未恢复的数据库状态。
