...
3. `docs/implementation/v0.0.3-plan.md`
4. `docs/implementation/v0.0.3-tasks.md`
5. `docs/implementation/v0.0.3-acceptance.md`

如果任务涉及 v0.0.3 UI/UX 设计、主题、首页、控制台或短链生成入口，必须继续阅读：

1. `docs/superpowers/specs/2026-06-08-v0.0.3-ui-ux-redesign.md`
2. `docs/product/scope-v0.0.3.md`
...
- 浅色主题采用「清透产品感」方向，深色主题采用「深墨绿清透」方向。
- 主题 token 应集中接入 Vuetify 3 主题系统，并保留跟随系统、浅色模式和深色模式切换能力。
...
- 控制台导航只展示当前已实现且有权限访问的入口，不用禁用菜单项提前占位。
- 我的短链、全站短链、用户管理和创建用户页面应迁移到统一控制台 Shell。
- UI 精细化不得破坏 v0.0.2 已完成的权限、筛选、搜索、用户管理和 E2E 主流程。

## 实施原则

- 以 v0.0.3 UI/UX 精细化重设计为当前优先目标。
- 实施前先确认对应产品范围、功能规格和技术基线。
- v0.0.3 只实现 `docs/product/scope-v0.0.3.md` 中列入必须实现的内容。
