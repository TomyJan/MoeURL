...
3. `docs/product/scope-v0.0.4.md`
...
3. `docs/implementation/v0.0.4-plan.md`
4. `docs/implementation/v0.0.4-tasks.md`
5. `docs/implementation/v0.0.4-acceptance.md`

如果任务涉及 v0.0.4 基础统计、短链访问事件、访问量、今日访问量或最近访问时间，必须继续阅读：
...
- v0.0.4 只做基础统计前置，不实现独立统计分析页、访问明细、来源、设备、地区、导出、访问密码、过期时间、中间页跳转、确认页跳转和二维码能力。
- v0.0.4 访问量只统计成功返回跳转响应的 `redirect_response_sent` 事件。
- 短链不存在、软删除、禁用或被访问条件拦截时，不计入访问量。
- `short_link_event` 只持久化短链 ID、事件类型和创建时间，不保存 IP、User-Agent、Referer 或地区信息。
- 我的短链列表和全站短链列表展示总访问量、今日访问量和最近访问时间。
- 统计展示必须沿用既有权限边界：普通用户只能看到自己的短链统计，管理员可在全站短链列表看到全站统计。
- 访问事件记录失败不得阻断短链跳转。
...
- v0.0.4 只实现 `docs/product/scope-v0.0.4.md` 中列入必须实现的内容。
