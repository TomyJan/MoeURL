...
create table moeurl_short_link_confirmation_permission_addition (
    user_group_id uuid not null references user_group(id) on delete cascade,
    permission text not null,
    primary key (user_group_id, permission)
);

comment on table moeurl_short_link_confirmation_permission_addition is
...
insert into moeurl_short_link_confirmation_permission_addition (user_group_id, permission)
...
set permissions = permissions - coalesce((
        ...
        from moeurl_short_link_confirmation_permission_addition as addition
...
drop table if exists moeurl_short_link_confirmation_permission_addition;
-- +goose StatementEnd
