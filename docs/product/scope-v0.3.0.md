...
- 公开预览仅返回短码、目标主机名、可选的中间页倒计时和可选过期时间，不返回密码状态、哈希或目标 URL。
...
GET /api/v1/public/short-link/preview?slug=...
  ...
  data.intermediateDelaySeconds: 3..10 或 null（直接跳转为 null）
  data.expiresAt: string 或 null

GET /go/{slug}/preview
  同源公共页面预览；密码状态由 `/go/{slug}` 的访问状态跳转原因决定
...
- 公开预览展示允许的主机名、倒计时和过期时间，不返回任何密码状态或 `targetUrl`；已授权列表响应保留 `targetUrl` 字段，并返回 `passwordEnabled` 布尔状态，不返回密码哈希。
