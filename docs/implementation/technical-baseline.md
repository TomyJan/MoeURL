...
v0.0.4 创建 `short_link_event` 表，用于持久化短链成功跳转事件，并支撑列表级基础统计。
...
- `short_link_event`：保存短链访问事件 ID、短链 ID、事件类型和创建时间。v0.0.4 只持久化用于基础统计的成功跳转事件，不保存 IP、User-Agent、Referer 或地区信息。
