...
数据库备份不包含 `.env`。应通过独立秘密管理流程备份生产配置；不要把明文 `.env` 与数据库备份放在同一公开位置。启用 OIDC 的部署必须保留原 `MOEURL_OIDC_ENCRYPTION_KEY` 和 `MOEURL_PUBLIC_BASE_URL`：数据库中的 provider Client Secret 使用该密钥加密，恢复数据库却缺少原密钥时，已配置 provider 无法登录。恢复演练应从受保护配置备份向 `restore.env` 注入同一 OIDC 加密密钥；公共地址则改为演练环境可达的 HTTPS Origin，并在身份提供商允许的回调地址范围内验证。
