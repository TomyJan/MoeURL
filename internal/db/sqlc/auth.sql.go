...
set failed_attempts = case
    ...
    blocked_until = case
        ...
        when attempt.failed_attempts + 1 >= $1::smallint then database_clock.value + interval '15 minutes'
        else attempt.blocked_until
    end,
    updated_at = database_clock.value
from database_clock
where attempt.username_hash = $2
...
type RecordAuthLoginFailureParams struct {
	FailureThreshold int16  `json:"failure_threshold"`
	UsernameHash     string `json:"username_hash"`
}

func (q *Queries) RecordAuthLoginFailure(ctx context.Context, arg RecordAuthLoginFailureParams) (AuthLoginAttempt, error) {
	row := q.db.QueryRow(ctx, recordAuthLoginFailure, arg.FailureThreshold, arg.UsernameHash)
