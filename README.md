...
可复制 `.env.example` 为 `.env` 后再按需调整 Compose 端口和运行环境；未提供 `.env` 时，Compose 使用 `docker-compose.yml` 中声明的插值默认值。当前这些默认值与 `.env.example` 保持一致属于项目维护约定，Compose 不会自动加载 `.env.example`。
